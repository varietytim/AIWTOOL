...
Interact_iGetPassword()
{
    ...
    do
        ...
        while :
        do
            read -p "Input your password again: " password2
            if [ ${#password2} -gt $maxlen ]; then
                stty echo
                Msg "The maximum length is $maxlen!"
                stty -echo
                continue
        ...
        if [ "$password" != "$password2" ]; then
            stty echo
            Msg "The password you input is not match, please try again"
            stty -echo
        else
            break
        fi
