...
Log_KeyValue()
{
    ...
    if [ ! -z $2 ] && [ "$2" = "append" ]; then
        echo "$1" >> $gWifiCmdDir/${gRunCmd}_kv.txt
    else
        echo "$1" > $gWifiCmdDir/${gRunCmd}_kv.txt
    fi
}
...
{
    local key="$1"
    local value=`cat $gWifiCmdDir/${gRunCmd}_kv.txt | grep -w "$key" | cut -d'=' -f 2`
...
Log_ShowCmdResult()
{
    [ -e "$gWifiCmdDir/${gRunCmd}.txt" ] && cat $gWifiCmdDir/${gRunCmd}.txt
}
