...
. $gDriverCommonDir/script/qmi_wwan_var.sh
...
GetInfoFromUser()
{
	...
	mode=$?
	    ...
	    gUsbmode=$usbmode
            ...
            # Enable QMI WWAN for monitor.sh
            Setup_iWriteUsrCfg $append "gCfgWWanEn=1"
            break
        fi
...
SetConfig()
{
    ...
    if [ $gUsbmode -eq $gPf_RmNetMode ]; then
    #
    # For QMI WWAN config
    #
        
        QmiCfg_iWriteCfg $gQmiApn $gUsername $gPassword $gAuth
        QmiCfg_iCopyToSystem

    elif [ $gUsbmode -eq $gPf_MbimMode ]; then
    #
    # For MBIM config
    #
...
InstallPackages()
{
    Package_iInstall
    if [ $? -ne 0 ]; then
        Msg "Try to install packages again."
        Package_iInstall
        [ $? -ne 0 ] && Msg "Install packages failed" && exit 1
    fi
}
