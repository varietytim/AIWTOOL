...
Aiw355_iConnect()
{
    ...
    cat $gATLogFile | grep -w 'NO CARRIER' > /dev/null
    if [ $? -eq 0 ]; then
        return $MErr_NOCARRIES
    fi
...
Aiw355_Init()
{
    ...
    local ret=`grep "^Aiw355_[^a-z].*" ${gModuleDir}/${gModuleName}/${gModuleName}.sh | cut -d'_' -f2 | sed 's/()//g' > $gModuleListFile`
    ret=$? && [ $ret -ne 0 ] && return $MErr_WRITELIST

    ProbeSerial 6 $gCfgPortSet
    ret=$? && [ $ret -ne 0 ] && gRetStr="Probe serial failed!" && return $ret

    Aiw355_l_WriteCfg "gDevNode=$gDevNode"
...
Aiw355_GetIP()
{
    ...
    #
    if [ $mode -eq 30 ] && [ "$status" = "0.0.0.0" ]; then    
...
Aiw355_SetPin()
{
    ...
    [ $# -le 0 ] && Aiw355_l_Help '<pin_string>' && return 0
...
Aiw355_SetApn()
{
    ...
    # Help information with this command
    [ $# -le 0 ] && Aiw355_l_Help '<apn_string>' && return 0
...
Aiw355_SetPinUnlock()
{
    ...
    # Help information with this command
    [ $# -le 0 ] && Aiw355_l_Help '<pin_code_string>' && return 0
...
Aiw355_SetAuth()
{
    ...
    # Help information with this command
    [ $# -lt 3 ] && Aiw355_l_Help '<username_string> <password_string> <auth 0:None, 1:PAP, 2:CHAP, 3:PAP and CHAP>' && return 0
...
Aiw355_SetLogLevel()
{
    ...
    # Help information with this command
    [ $# -le 0 ] && Aiw355_l_Help "<$L_LEVEL0, $L_LEVEL1 or $L_LEVEL2>" && return 0
