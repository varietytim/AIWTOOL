...
Ewm341_Init()
{
    ...
    local ret=`grep "^Ewm341_[^a-z].*" ${gModuleDir}/${gModuleName}/${gModuleName}.sh | cut -d'_' -f2 | sed 's/()//g' > $gModuleListFile`
    ret=$? && [ $ret -ne 0 ] && return $MErr_WRITELIST

    ProbeSerial 6 $gCfgPortSet
    ret=$? && [ $ret -ne 0 ] && gRetStr="Probe serial failed!" && return $ret

    Ewm341_l_WriteCfg "gDevNode=$gDevNode"
...
Ewm341_GetIP()
{
    ...
    if [ $ret -eq 0 ]; then
        ...
        cid=`grep $gCfgApn $gATLogFile | cut -d',' -f1 | sed 's/\"//g' | sed 's/\r//g' | cut -d':' -f2 | tr -d ' ' | head -n 1`
        [ -z "$cid" ] && cid="1"
    else
...
Ewm341_GetPinCount()
{
    ...
    local status=`cat $gATLogFile | grep "SIM PIN," | cut -d',' -f2 | sed 's/\r//g'| head -n 1` 
    [ -z "$status" ] && return $MErr_PPINCNT

    Log_ModuleCmd "[+CPINR]: $status"
    Log_ShowCmdResult
...
Ewm341_SetPin()
{
    ...
    [ $# -le 0 ] && Ewm341_l_Help '<pin_string>' && return 0
...
Ewm341_SetApn()
{
    ...
    # Help information with this command
    [ $# -le 0 ] && Ewm341_l_Help '<apn_string>' && return 0
...
Ewm341_SetPinUnlock()
{
    ...
    # Help information with this command
    [ $# -le 0 ] && Ewm341_l_Help '<pin_code_string>' && return 0
...
Ewm341_SetAuth()
{
    ...
    # Help information with this command
    [ $# -lt 3 ] && Ewm341_l_Help '<username_string> <password_string> <auth 0:None, 1:PAP, 2:CHAP>' && return 0
...
Ewm341_SetLogLevel()
{
    ...
    # Help information with this command
    [ $# -le 0 ] && Ewm341_l_Help "<$L_LEVEL0, $L_LEVEL1 or $L_LEVEL2>" && return 0
